...
# Role/risk alias for the canonical implementation at scripts/ai/repomix-ensure-fresh.sh.
# Provides scripts/ai/bin/context/repomix-ensure-fresh.sh addressability without relocating
...
# Resolve the scripts/ai root: this shim lives at scripts/ai/bin/context/repomix-ensure-fresh.sh,
# so the root is two directories up (context -> bin -> scripts/ai).
_ai_shim_dir="$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
_ai_root="$(CDPATH= cd -- "$_ai_shim_dir/../.." && pwd)"
_ai_impl="$_ai_root/repomix-ensure-fresh.sh"
