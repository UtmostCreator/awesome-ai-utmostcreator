...
# Role/risk alias for the canonical implementation at scripts/ai/install-mandatory-tools.sh.
# Provides scripts/ai/bin/admin/install-mandatory-tools.sh addressability without relocating
...
# Resolve the scripts/ai root: this shim lives at scripts/ai/bin/admin/install-mandatory-tools.sh,
# so the root is two directories up (admin -> bin -> scripts/ai).
_ai_shim_dir="$(CDPATH='' cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
_ai_root="$(CDPATH='' cd -- "$_ai_shim_dir/../.." && pwd)"
_ai_impl="$_ai_root/install-mandatory-tools.sh"
