...
# Role/risk alias for the canonical implementation at scripts/ai/ai-rollback.sh.
# Provides scripts/ai/bin/edit/ai-rollback.sh addressability without relocating
...
# Resolve the scripts/ai root: this shim lives at scripts/ai/bin/edit/ai-rollback.sh,
# so the root is two directories up (edit -> bin -> scripts/ai).
_ai_shim_dir="$(CDPATH='' cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
_ai_root="$(CDPATH='' cd -- "$_ai_shim_dir/../.." && pwd)"
_ai_impl="$_ai_root/ai-rollback.sh"
