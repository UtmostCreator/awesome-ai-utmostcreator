...
# Role/risk alias for the canonical implementation at scripts/ai/ai-install-coverage.sh.
# Provides scripts/ai/bin/verify/ai-install-coverage.sh addressability without relocating
...
# Resolve the scripts/ai root: this shim lives at scripts/ai/bin/verify/ai-install-coverage.sh,
# so the root is two directories up (verify -> bin -> scripts/ai).
_ai_shim_dir="$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
_ai_root="$(CDPATH= cd -- "$_ai_shim_dir/../.." && pwd)"
_ai_impl="$_ai_root/ai-install-coverage.sh"
