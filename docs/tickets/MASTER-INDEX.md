...
| — | markdown-surface-reduction | Phase 1: `git rm -r docs/tickets/archive` (16 files, clean tree, isolated commit) — **done** (`0c0f219`); Phase 2: read-only investigate `tools/ai/tools/{actions,examples}` (30 files, doc-only refs confirmed, zero runtime/validator coupling) — evidence gathered, keep/consolidate/regenerate decision still pending. Capability sub-files + adapters + templates KEPT (runtime contract) | none (Phase 1 done); human decision (Phase 2) | low | **Phase 1 done; Phase 2 awaiting keep/consolidate/regenerate decision** |
