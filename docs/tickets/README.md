...
- Durable AI-generated plans live here too, one folder per current git branch: `docs/tickets/{branch-name}/plan-{n}-{short-desc}.md` (numbered `plan-1-...`, `plan-2-...`, ... when multiple tickets are planned on the same branch). On completion, the plan is renamed `DONE-plan-{n}-{short-desc}.md` and moved into `docs/tickets/{branch-name}/archive/`. This keeps generated plans committed instead of in the gitignored `docs/ai/generated/`.
