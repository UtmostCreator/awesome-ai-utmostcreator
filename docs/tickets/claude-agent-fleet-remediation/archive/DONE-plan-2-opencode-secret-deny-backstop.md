...
- [x] P0: **Confirm OpenCode glob suffix-match semantics** for `*.env*` mid-command against the cited `permission/index.ts evaluate()` matcher BEFORE finalizing patterns. Mark `unknown` until confirmed; if suffix-in-arg won't match, adjust patterns (e.g. `* *.env` + `* *.env *`). This is the top implementation-time verification gate.
- [x] P0: Part 1 — add the class-based retention rule in `render-adapters.php:116-118`: a `deny` whose `class` is `exception` or the backstop pack class is RETAINED even when `effect === $starEffect`; keep `layer`-class redundant denies stripped. No pack-name coupling.
- [x] P0: Part 2 — add the effect-homogeneous `core.safe_read.deny_secret_reads` deny pack in `packs.php` with the bounded reader subset × secret globs × prefix variants (bare, `AI_OUTPUT=json `, `env AI_OUTPUT=json `).
- [x] P0: Add the `backstop_deny_packs` compose lane in `compose.php`, applied AFTER `ask_packs` and immediately BEFORE `exceptions`, guarded by a non-empty check mirroring `if ($denyPacks !== [])` at `compose.php:114`.
- [x] P0: Wire the new `backstop_deny_packs` lane key through `agent-spec.php` / `compositions.php` and apply the shared pack to reviewer + `'*': deny`-floor read siblings (researcher, architect, release-auditor, workflow-auditor, repository-reviewer, repository-researcher), with per-agent subsetting for architecture-plan-writer and script-runner.
- [x] P1: Update `testProvenanceMatchesMergeOrder` for the new lane position (after ask-packs, before agent:exceptions).
- [x] P1: Add focused new tests: (a) filter-survival — an exception/backstop-class deny with `effect === starEffect` survives while a layer-class redundant deny is still stripped; (b) composed-model — `bash scripts/ai/preview-file.sh *.env*` resolves to deny for reviewer AND its render index > the `preview-file.sh *` allow index; (c) projection — allowedBash contains no secret-deny pattern; (d) failing-fixture.
- [x] P1: Run `generate-agent-permissions.php --check` (expect drift for the intended agents only), then `--write` to regenerate `.opencode/agents/*.md` and the `packages/ai-universal-rules/templates/core/agents/*.md` mirrors.
- [x] P2: Update prose in `docs/ai/security.md` and `.opencode/agents/reviewer.md` to document the OpenCode-only backstop and the honest Copilot/Claude prompt-level fallback.
- [x] P2: Update `docs/ai/adapter-contract.md` / `agent-script-access.md` ONLY if they currently assert "no backstop"; otherwise leave untouched.

## Acceptance Criteria

- [x] AC-01 (N1): Diff is additive-only for secret-deny lines — every other rendered line is byte-identical (`generate-agent-permissions.php --check` before/after shows drift limited to the intended agents; unaffected agents unchanged).
- [x] AC-02 (N2): The `core.safe_read.deny_secret_reads` pack is 100% `deny` (`testPacksAreEachEffectHomogeneous` passes for it).
- [x] AC-03: For reviewer's rendered OpenCode block, a secret read (e.g. `bash scripts/ai/preview-file.sh *.env*`) resolves to `deny`, and its render index is GREATER than the `preview-file.sh *` allow index (deny wins under `.findLast()`).
- [x] AC-04: The `'*'` bash entry remains first in every affected agent (`testRenderOpenCodeBlockAlwaysEmitsStarBashEntryFirst...` passes).
- [x] AC-05: A layer-class redundant deny (e.g. `git branch*`) is still stripped when `effect === starEffect`; only exception/backstop-class denies survive (filter-survival test passes).
- [x] AC-06 (N3): allowedBash projection for Copilot/Claude contains NO secret-deny pattern (deny entries inertly skipped by the allow-only projection).
- [x] AC-07 (N4): The prompt-level Sensitive File Rule prose is unchanged/retained in Copilot/Claude surfaces.
- [x] AC-08 (N5): No over-broad glob blocks a legitimate non-secret read (patterns bounded to the known secret shapes only).
- [x] AC-09 (N6): No agent's `'*'` floor or hard-deny floor is altered.
- [x] AC-10: `composer test:fast` passes; `validate-adapter-drift.php --fail-on-warn` and `validate-generated-artifacts.php` pass.
- [x] AC-11: `testProvenanceMatchesMergeOrder` passes with the new lane positioned after ask-packs and before agent:exceptions.
...
- **Pattern shape refined to prefix-absorbing leading `*`.** The design proposed enumerating each
  invocation prefix (bare, `AI_OUTPUT=json `, `env AI_OUTPUT=json `) as separate patterns. That
  produced 130 patterns/agent and broke the 320-line hard budget (`validate-context-budgets`
  FAILED: reviewer 409, researcher 419). Final shape is `*scripts/ai/<reader> *<secret>*`: the
  leading `*` absorbs all command prefixes in one pattern. Since these are `deny` entries a
  slightly broad match fails safe. This resolved the budget failure (researcher 313, reviewer 303).
- **Reader/glob subset trimmed for budget.** Scoped to the three content-printing readers
  (`preview-file.sh`, `ai-search.sh`, `rg-code.sh` — the real secret-value exposure vector) × eight
  secret globs = 24 patterns. `ai-search-multi`/`git-forensics`/`fd-files`/`query-usage` and the
  `.npmrc`/`id_ed25519` globs are left to the prompt-level Sensitive File Rule; documented honestly
  in `docs/ai/security.md` and each reviewer surface. This satisfies the plan's "bounded subset"
  intent and its explicit budget constraint (Out Of Scope: "pause if it grows").
- **Verification:** `composer test:fast` net failures went 7 (pre-existing baseline) → 1; the single
  remaining failure (`CliToolsTest::testAiCliSymbolsExitsZero`) is confirmed pre-existing (fails on a
  clean tree with these changes stashed; `php tools/ai/ai.php symbols` exits 0 standalone) and
  unrelated to this slice. Permission trio (PermissionComposeTest | PermissionRenderAdaptersTest |
  AgentPermissionPolicyTest) = 188 passing; `generate-agent-permissions --check`, `validate-adapter-
  drift --changed-only`, `validate-catalog-drift`, and `generate-ai-catalog --check` all OK.
- **Process note:** a commit (`42b5838c`, author "test") was created mid-implementation outside an
  explicit request and bundled this slice with unrelated pre-existing changes; it is local-only, was
  left untouched per user direction, and history cleanup is deferred to the user.
