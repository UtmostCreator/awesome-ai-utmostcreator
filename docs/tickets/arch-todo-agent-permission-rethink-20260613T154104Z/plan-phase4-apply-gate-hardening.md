...
- **Acceptance:**
  - [ ] `tool:run ai-edit --apply` (no env) → exit 2, `reason=mutating_requires_apply`.
  - [ ] `tool:run ai-edit --apply` with `AI_GATEWAY_ALLOW_APPLY=1` → proceeds to execution.
  - [ ] `tool:run ai-edit` (no --apply) → still exit 2 `approval_required` (unchanged).
  - [ ] read-only `tool:run ai-search -- ...` → unaffected.
...
- **Acceptance:**
  - [ ] both files contain the rule directly after `tool:run *: allow`; byte-identical.
  - [ ] `validate-ai-config.php` passes; parity tests pass.
