...
- **Acceptance:**
  - [x] one function returns a normalized entry; gateway `tool:describe` and the exporter both call it.
  - [x] derived fields are computed, never hand-stored.
  - [x] `ToolGatewayTest` stays green (assert new fields present).
...
- **Acceptance:**
  - [x] generated JSON is byte-stable across runs (stable key order, fixed pretty-print).
  - [x] current `docs/ai/script-registry.json` regenerates to an equal or richer file.
  - [x] `registry:export --check docs/ai/script-registry.json` passes in CI; edited JSON fails it.
...
- **Acceptance:**
  - [x] schema validates the P3.1 output; `tier` stays optional; `risk` no longer allows low/medium/high.
