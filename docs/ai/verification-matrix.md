...
| `bash scripts/ai/ai-verify-{php,js,ts,vue,html}.sh .`                                                     | Optional manual-only narrowing of the `ai-verify.sh` gate to one language (`ai-verify.sh --language <lang>`); **not agent-wired**, human-invoked only | depends on installed tools; same budget as `ai-verify.sh`                 | `scripts/ai/ai-verify-php.sh` (and per-language siblings)           |
