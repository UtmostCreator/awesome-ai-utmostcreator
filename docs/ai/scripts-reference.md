...
| `ai-verify-html.sh` | Manual-only convenience wrapper narrowing verification to HTML (`ai-verify.sh --language html`); not agent-wired. | Lets a human run just the HTML-surface checks without the full pipeline. | Verification report; read-only but ask-gated by policy. | `--help`/`-h`/`--introspect` delegate to `ai-verify.sh`. |
| `ai-verify-js.sh` | Manual-only convenience wrapper narrowing verification to JS (`ai-verify.sh --language js`); not agent-wired. | Lets a human run just the JS-surface checks without the full pipeline. | Verification report; read-only but ask-gated by policy. | `--help`/`-h`/`--introspect` delegate to `ai-verify.sh`. |
| `ai-verify-php.sh` | Manual-only convenience wrapper narrowing verification to PHP (`ai-verify.sh --language php`); not agent-wired. | Lets a human run just the PHP-surface checks without the full pipeline. | Verification report; read-only but ask-gated by policy. | `--help`/`-h`/`--introspect` delegate to `ai-verify.sh`. |
| `ai-verify-ts.sh` | Manual-only convenience wrapper narrowing verification to TS (`ai-verify.sh --language ts`); not agent-wired. | Lets a human run just the TS-surface checks without the full pipeline. | Verification report; read-only but ask-gated by policy. | `--help`/`-h`/`--introspect` delegate to `ai-verify.sh`. |
| `ai-verify-vue.sh` | Manual-only convenience wrapper narrowing verification to Vue (`ai-verify.sh --language vue`); not agent-wired. | Lets a human run just the Vue-surface checks without the full pipeline. | Verification report; read-only but ask-gated by policy. | `--help`/`-h`/`--introspect` delegate to `ai-verify.sh`. |
...
| `run-test-focused.sh` | Run a focused PHPUnit selection (`--filter` or a single test file), bounded by `TEST_TIMEOUT`. | Proves one class/method fast without the whole suite. | Test output; read-only; hard timeout. | `--help`/`-h` (introspect help view); requires a filter or file arg. |
...
| `ship-audit.sh` | Audit installer pack targets for forbidden shipping paths. | Catches repo-local/generated/dependency directories before packaging. | Read-only report; caveat: ignored generated dirs may be untracked. | `--help`/`-h`. |
