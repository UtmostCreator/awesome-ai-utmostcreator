...
- `scripts/ai/run-repomix-file.sh` — exact single-file Repomix wrapper. Packs one repository-relative file via `repomix --stdin`; defaults to `--compress --style xml` and writes to the generated single-file context output area unless `--output` is supplied.
