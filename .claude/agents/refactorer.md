...
- `git diff*`
- `git log*`
- `git show*` / `git blame*` / `git rev-parse*` / `git ls-files*` — git-forensic read commands; see `docs/ai/agent-script-access.md` for per-script guidance.
- `bash scripts/ai/ai-search.sh *` — plain, `AI_OUTPUT=json`, and `env AI_OUTPUT=json` prefixed forms all approved.
- `bash scripts/ai/ai-search-multi.sh *` — plain, `AI_OUTPUT=json`, and `env AI_OUTPUT=json` prefixed forms all approved.
- `bash scripts/ai/preview-file.sh *` — plain, `AI_OUTPUT=json`, and `env AI_OUTPUT=json` prefixed forms all approved.
...
- `AI_VERIFY_SCOPE=changed VERIFY_SECRETS=0 bash scripts/ai/ai-verify.sh *` — plain and `env`-prefixed forms both approved (see Script Access for the settings.json caveat).
- `lychee *`
- `actionlint*`
- `shfmt -d *`
- `shellcheck *`
